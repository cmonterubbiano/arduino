...
void setup()
{
  ...
  SIM900.println("AT+CMGF=1");  // Imposta la modalità SMS a testo
  delay(200);
  SIM900.println("AT+CNMI=2,2,0,0,0"); // trasferisce il contenuto di un nuovo messaggio 
                                       // SMS all'uscita seriale del modulo GSM.
  delay(200);
  SIM900.println("AT+CSCS=\"GSM\"\r"); // Imposta la codifica GSM
  delay(200);
}
