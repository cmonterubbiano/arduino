...
int  attesa =10;    // 40 condizioni normali se attesa prima chiamata e' 35 secondi
int  secondi =0;
int  tempomax =20;  // dovrebbe essere cirva 300 ossia 5 minuti
char  mess[17];
int  spegni =10;
int  citofono =8;
int  pulsante =9;
...
//    Serial.print(mess);
  ...
  if (secondi >=tempomax)
  {
    ...
    // hanno senso solo con alimentazione da USB 
    for (attesa =10; attesa; attesa--)
    {
      Serial.println("TOLGO ALIMENTAZIONE");
      digitalWrite(spegni, LOW);
      delay(1000);
      digitalWrite(spegni, HIGH);
       delay(1000);
