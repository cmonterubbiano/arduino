...
*/
#define T_MAX 30000   // Tempo massimo apertura/chiusura serranda
#define P_MIN 3       // Tempo minimo di pressione pulsante in millisecondi
...
void setup() 
{
  ...
  for (ix =0; ix <n_senrele[iy]; ix++)
  {
	...
	// Serial.print("OUTPUT ");
	// Serial.print(a_rele[ix]);
	delay(20);
	digitalWrite(a_rele[ix], (!a_roff[ix] ? LOW : HIGH));
	// Serial.print(" OFF ");
	// Serial.println((!a_roff[ix] ? "LOW" : "HIGH"));
	delay(20);
  }
  iy++;
  for (ix =0; ix <n_senrele[iy]; ix++)
  {
	...
	// Serial.print("OUTPUT ");
	// Serial.print(c_rele[ix]);
	delay(20);
	digitalWrite(c_rele[ix], (!c_roff[ix] ? LOW : HIGH));
	// Serial.print(" OFF ");
	// Serial.println((!c_roff[ix] ? "LOW" : "HIGH"));
	delay(20);
  }
  for (ix =0; ix <6; ix++)
  {
...
void loop() 
{
	...
	|| ((millis() -t_ultlet) >=10000L))
	{
		...
		{
			diverso =0;
			Serial.print("Sensori finestre =");
			for (ix =0; ix <n_senrele[iy]; ix++)
			{
				Serial.print(" \t");
				Serial.print(f_status[ix]);
			}
			Serial.println("");
		}
		iy++;
		...
		{
			diverso =0;
			Serial.print("Sensori serrande =");
			for (ix =0; ix <n_senrele[iy]; ix++)
			{
				Serial.print(" \t");
				Serial.print(s_status[ix]);
			}
			Serial.println("");
		}
		iy++;
		...
		{
			diverso =0;
			Serial.print("Sensori porte =");
			for (ix =0; ix <n_senrele[iy]; ix++)
			{
				Serial.print(" \t");
				Serial.print(p_status[ix]);
			}
			Serial.println("");
		}
		Serial.print("Fine: ");
	...
	for (ix =0 ; ix <6; ix++)
	{
		...
		if (ix !=5
		&& c_tempo[ix]
		&& (millis() -c_tempo[ix]) >=2000	// serranda in chiusura con sensore
		&& !digitalRead(s_sensori[ix]))		// gia' chiuso continuo x 1 secondo
		{
			Serial.print("Chiuso: ");
			Serial.print(ix);
			Serial.print(" tempo: ");
